...
class TestTailorPdf:
    ...
    class TestCreateTailoredResume:
        ...
        @pytest.mark.parametrize("resume_object", ["test_arvind_resume_original.pdf"], indirect=True)
        def test_compare_change_in_pdf(self, resume_object):
            bullets_to_redact = ["A deliberate break to recharge, explore my bucket-list destinations, and spend quality time with family.",]
            tailor_pdf = TailorPdf(resume_object, bullets_to_redact)
            tailor_pdf.create_tailored_resume()
            tailored_doc = tailor_pdf.tailored_resume
            # filepath = Path(__file__).with_name("tailored_resume.pdf")
            # tailor_file = open(filepath, 'w+')
            # tailored_doc.save(tailor_file)

            filepath = Path(__file__).parent / "test_media_root/uploads/resumes/test_arvind_resume_updated_one_bullet_v0.pdf"
            print(filepath)
            expected_doc = pymupdf.open(filepath)
            for page_no in range(1, tailored_doc.page_count):
                expected_doc_text_blocks, tailored_resume_text_blocks = expected_doc[page_no].get_text("blocks"), tailored_doc[page_no].get_text("blocks")
                assert expected_doc_text_blocks == tailored_resume_text_blocks
            # for page in range(len(tailored_resume.page_count):


        @pytest.mark.parametrize("resume_object", ["test_arvind_resume_original.pdf"], indirect=True)
        def test_compare_change_in_pdf_line_overlap_bottom(self, resume_object):
            bullets_to_redact = [
                "A deliberate break to recharge, explore my bucket-list destinations, and spend quality time with family.",
                "Built interactive React dashboards to visualize learner progress and ROI for enterprise clients.",
                "Developed public-facing digital news websites using Django, React, Typescript, and MySQL.",
                "Upgraded the Press Releases product from HTML to React/Typescript  – led to 300% increase in product revenue.",
            ]
            tailor_pdf = TailorPdf(resume_object, bullets_to_redact)
            tailor_pdf.create_tailored_resume()
            tailored_doc = tailor_pdf.tailored_resume
        ...
        @pytest.mark.parametrize("resume_object", ["test_arvind_resume_original.pdf"], indirect=True)
        def test_compare_change_in_pdf_line_overlap_top(self, resume_object):
            bullets_to_redact = [
                "A deliberate break to recharge, explore my bucket-list destinations, and spend quality time with family.",
                "Delivered critical features for a large-scale LMS platform using Django, React, TypeScript, and Postgres.",
                "Built interactive React dashboards to visualize learner progress and ROI for enterprise clients.",
                "Developed public-facing digital news websites using Django, React, Typescript, and MySQL.",
                "Upgraded the Press Releases product from HTML to React/Typescript  – led to 300% increase in product revenue.",
                "Implemented user-requested features including bulk order processing and Stripe payment integration.",
                "Improved team debugging workflows by enabling IDE-based debugging for Dockerized applications.",
            ]
            tailor_pdf = TailorPdf(resume_object, bullets_to_redact)
            tailor_pdf.create_tailored_resume()
            tailored_doc = tailor_pdf.tailored_resume
        ...
        def test_compare_test_resumes(self, tailor_pdf):
            filepath = Path(
                __file__).parent / f"test_media_root/uploads/resumes/{tailor_pdf.template_resume.filename()}"
            print(filepath)
            expected_doc = pymupdf.open(filepath)

            tailor_pdf.create_tailored_resume()
            tailored_doc = tailor_pdf.tailored_resume
            for page_no in range(tailored_doc.page_count):
                tailor_page, expected_page = tailored_doc[page_no], expected_doc[page_no]
            #     assert len(tailor_page.get_fonts()) == len(expected_page.get_fonts())
            #     assert len(tailor_page.get_links()) == len(expected_page.get_links())
                print(len(tailor_page.get_fonts()))
                print(len(expected_page.get_fonts()))
                print("––––––––––––––––––––")


            filepath = Path(__file__).with_name(f"tailored_resume_{len(tailor_pdf.column_data)}.pdf")
            tailor_file = open(filepath, 'w+')
            tailored_doc.save(tailor_file)
